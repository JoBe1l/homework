...
# (с округлением до целого) деление клеток, соответственно. Сложение. Объединение двух клеток. При этом число ячеек
# общей клетки должно равняться сумме ячеек исходных двух клеток. Вычитание. Участвуют две клетки. Операцию
# необходимо выполнять только если разность количества ячеек двух клеток
...
# необходимо реализовать метод make_order(), принимающий экземпляр класса и количество ячеек в ряду.
...
class Cell:
    ...
    def __sub__(self, other):
        if self.cell - other.cell <= 0:
            return "Отрицательная клетка не может появиться"
        else:
            return "*" * (self.cell - other.cell)

    def __truediv__(self, other):
        if self.cell / other.cell < 1:
            return "Меньшее число клеток нельзя делить на большее"
        else:
            return "*" * int(self.cell / other.cell)

    def make_order(self, row):
        k = self.cell % row
        for i in range(0, int(self.cell / row)):
            print("*" * row)
        print("*" * k)
